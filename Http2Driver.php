...
final class Http2Driver implements HttpDriver, Http2Processor
{
    ...
    public function handlePong(string $data): void
    {
        ...
        $rtt = $this->client->rtt[$data];
        $t = microtime(1);
        $this->client->rtt[$data] = [$rtt[0],$t,$t - $rtt[0]];
    ...
    {
        $pseudo_arr = [];
        foreach ($pseudo as $k=>$p){

            $pseudo_arr[] = substr($k,1,1);
        }


        $this->headleH2PF('PS[,]',implode(',',$pseudo_arr));
    ...
    {
        if($weight<=256){
            // from  parsePriorityFrame
            $this->headleH2PF('P[,]',implode(':',[$streamId,0,$parentId,$weight]),'push');
        }else{
            // from  parseHeaders
            $weight -= 256;
        }
    ...
    public function handleSettings(array $settings): void
    {
        ...
        $this->headleH2PF('S[;]',implode(';',$settings_arr));
    ...
    public function headleH2PF($k,$v,$type = 'set')
    {

        if(!isset($this->client->h2fp)){
            @$this->client->h2fp = [];
        }

        if(isset($this->client->h2fp['PS[,]'])){
            return;
        }
        if($type=='set'){
            $this->client->h2fp[$k] = $v;
        }else if($type = 'push'){
            !isset($this->client->h2fp[$k]) && $this->client->h2fp[$k] = [];
            $this->client->h2fp[$k][] = $v;
        }
    }
}
